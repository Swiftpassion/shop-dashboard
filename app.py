...
try:
    ...
    # --- PAGE 7: MASTER_ITEM (NEW) ---
    elif selected_page == "🔧 MASTER_ITEM":
        st.markdown('<div class="header-bar"><div class="header-title"><i class="fas fa-tools"></i> จัดการ Master Item </div></div>', unsafe_allow_html=True)
        
        # ฟังก์ชันสำหรับโหลดและบันทึกข้อมูล
        ...
        try:
            ...
            st.error(f"ไม่สามารถโหลดข้อมูลจาก MASTER_ITEM ได้: {e}")
            st.stop()

        # กำหนดคอลัมน์ที่ต้องการให้แสดงและแก้ไข (ตามที่คุณระบุ)
        # หมายเหตุ: เราจะแสดง SKU และ ชื่อสินค้า ด้วยเพื่อให้รู้ว่าแก้ตัวไหน แต่ SKU ควรระวังการแก้ไข
        target_columns = [
            'SKU', 'ชื่อสินค้า', 
            'ทุน', 'ราคากล่อง', 'ค่าส่งเฉลี่ย', 
            'ค่าคอมมิชชั่น Admin', 'ค่าคอมมิชชั่น Telesale', 
        ...
        # ตรวจสอบว่าคอลัมน์ไหนมีในไฟล์จริงบ้าง (กัน Error กรณีชื่อคอลัมน์ไม่ตรงเป๊ะ)
        available_cols = [c for c in target_columns if c in df_master_edit.columns]
        
        # ถ้ามีคอลัมน์อื่นที่ไม่ได้ระบุ แต่อยากให้มันติดไปด้วยเวลากดเซฟ (เพื่อไม่ให้ข้อมูลหาย)
        other_cols = [c for c in df_master_edit.columns if c not in available_cols]
        final_cols_order = available_cols + other_cols
        
        # จัดเรียงคอลัมน์ใหม่
        df_editor_view = df_master_edit[final_cols_order].copy()

        st.info("💡 **คำแนะนำ:** แก้ไขข้อมูลในตารางด้านล่าง แล้วกดปุ่ม **'บันทึกข้อมูลลง Google Sheet'** ที่ด้านล่างสุด")

        # แสดง Editor
        # num_rows="dynamic" จะช่วยให้คุณเพิ่มแถวใหม่ (สินค้าใหม่) ได้ด้วย
        edited_df = st.data_editor(
            ...
            height=600,
            column_config={
                "ทุน": st.column_config.NumberColumn(format="%.2f"),
                "ราคากล่อง": st.column_config.NumberColumn(format="%.2f"),
                "ค่าส่งเฉลี่ย": st.column_config.NumberColumn(format="%.2f"),
                "ค่าคอมมิชชั่น Admin": st.column_config.NumberColumn(format="%.4f", help="ใส่ทศนิยม เช่น 0.05 คือ 5%"),
                "ค่าคอมมิชชั่น Telesale": st.column_config.NumberColumn(format="%.4f", help="ใส่ทศนิยม เช่น 0.05 คือ 5%"),
            }
        )

        st.markdown("---")
        
        col_save, col_dummy = st.columns([2, 5])
        
        with col_save:
            if st.button("💾 อัพเดทข้อมูล", type="primary", use_container_width=True):
                try:
                    with st.spinner("กำลังบันทึกข้อมูล... กรุณารอสักครู่"):
                        # แปลง DataFrame กลับเป็น List of Lists เพื่อเตรียมบันทึก
                        # ต้องระวังเรื่อง NaN (ค่าว่าง) ให้เป็น String ว่าง หรือ 0 ตามความเหมาะสม
                        
                        # แปลงค่า Numeric ที่เป็น NaN ให้เป็น 0 หรือ "" ก่อนบันทึกตาม Logic ของคุณ
                        # ในที่นี้แปลงเป็น String เพื่อความปลอดภัยในการเขียนลง Sheet ไม่ให้ Error
                        save_df = edited_df.fillna("")
                        
                        # เตรียมข้อมูล (รวม Header)
                        vals = [save_df.columns.values.tolist()] + save_df.values.tolist()
                        
                        # อัปเดตข้อมูลทั้งหมดลง Sheet (Clear ของเก่าแล้วลงใหม่)
                        ws.update(range_name='A1', values=vals)
                        
                        st.success("✅ บันทึกข้อมูลเรียบร้อยแล้ว!")
                        st.cache_data.clear() # เคลียร์ Cache เพื่อให้หน้าอื่นเห็นข้อมูลใหม่
