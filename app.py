    ...
    # --- PAGE 2: REPORT_DAILY ---
        ...
        if df_final_d.empty: st.warning(f"⚠️ ไม่พบข้อมูลตามเงื่อนไข ({sel_category_d}) ในช่วงเวลานี้")
            ...
            for i, (_, r) in enumerate(df_final_d.iterrows()):
                ...
                html += f'<td style="text-align:left;font-size:11px;color:#1e3c72 !important; max-width: 130px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{r["ชื่อสินค้า"]}">{r["ชื่อสินค้า"]}</td>'
