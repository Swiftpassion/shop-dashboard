...
# ==========================================
# 1. CONFIG & CSS (ยกมาจาก Cell 2 100%)
# ==========================================
...
<style>
    ...
    .table-wrapper {
        ...
        padding-bottom: 10px; position: relative; border: 1px solid #444;
    }
    .custom-table {
    ...
    .kpi-card-pnl {
        background-color: #1c1c1c; border: 1px solid #333; border-radius: 12px; padding: 20px;
...
# ==========================================
# 3. BACKEND: DATA LOADING & PROCESSING (Cell 1 Logic + API)
# ==========================================
def safe_float(val):
...
# ==========================================
# 5. FRONTEND: UI (Cell 2 Logic - Mapped 1:1)
# ==========================================
try:
    ...
    # --- PAGE 2 ---
        ...
        if df_final_d.empty: st.warning("⚠️ ไม่พบข้อมูลตามเงื่อนไขในช่วงเวลานี้")
            ...
            sum_profit = df_final_d['Net_Profit'].sum() - estimated_fix_cost
    ...
    # --- PAGE 5 ---
        ...
        m_tele = df_d_agg['CAL_COM_TELESALE'].sum()
        m_ads = df_d_agg['Ads_Amount'].sum()
        # fix_cost_month defined earlier
