...
try:
    ...
    # --- PAGE 7: MASTER_ITEM (NEW UI) ---
        ...
        c_info, c_btn = st.columns([3.5, 1.5]) # แบ่งสัดส่วนพื้นที่ (ซ้ายเยอะกว่า)

        with c_info:
            st.info("💡 **คำแนะนำ:** แก้ไขข้อมูลในตารางด้านล่าง แล้วกดปุ่ม **'บันทึกข้อมูล'** ทางขวามือ ➔")
        
        with c_btn:
            # ใช้ Div ดันปุ่มลงมาเล็กน้อยให้ตรงกับกล่องข้อความ
            st.markdown('<div style="margin-top: 0px;"></div>', unsafe_allow_html=True)
            # สร้างตัวแปรรับค่าการกดปุ่ม (ยังไม่ทำงานทันที รอข้อมูลจาก Editor ด้านล่าง)
        ...
        # --- 5. Logic การบันทึก (ตรวจสอบว่าปุ่มถูกกดหรือไม่) ---
        if click_save:
            try:
                with st.spinner("⏳ กำลังบันทึกข้อมูล... กรุณารอสักครู่"):
                    ...
                    # Rerun เพื่อให้ปุ่มเด้งกลับและข้อมูลรีเฟรช
                    # st.rerun() 
